...
class MasterConfig(commands.Cog):
    ...
    async def create_tables(self):
        """
        Create the configuration table and migrate older versions.
        ...
        async with self.pool.acquire() as connection:
            ...
            # Create the current table when it does not exist.
            ...
            # Add legacy/current columns when an older table already exists.
            await connection.execute(
                """
                ALTER TABLE bot_config_files
            ...
            # Old: filename -> New: config_path
            await connection.execute(
                """
                UPDATE bot_config_files
                SET config_path = filename
                WHERE
                    (config_path IS NULL OR config_path = '')
                    AND filename IS NOT NULL
                """
            )

            # Old: content -> New: config_data
            ...
            for row in legacy_rows:
                ...
                try:
                    ...
                    print(
                        f"⚠️ Could not migrate legacy "
                        f"configuration {filename}: {e}"
                    )

            # Populate legacy columns from current columns where necessary.
            await connection.execute(
                """
                UPDATE bot_config_files
                SET filename = config_path
                WHERE
                    filename IS NULL
            ...
            # Remove unusable empty-path records.
            await connection.execute(
                """
                DELETE FROM bot_config_files
                WHERE
                    config_path IS NULL
                    OR config_path = ''
                """
            ...
            # The old primary key can remain on filename.
            # This unique index lets ON CONFLICT(config_path) work.
            await connection.execute(
                """
                CREATE UNIQUE INDEX IF NOT EXISTS
                bot_config_files_config_path_unique
                ON bot_config_files (config_path)
                """
            )

        print("✅ bot_config_files table ready")
        print("🔄 Database schema compatibility check complete")
    ...
    async def restore(
        ...
        try:
            # Restore every configuration stored in Supabase.
            await self.restore_all_configs()

            # Verify how many configurations are currently stored.
            async with self.pool.acquire() as connection:
                count = await connection.fetchval(
                    """
                    SELECT COUNT(*)
            ...
            await interaction.followup.send(
                ...
                "📂 Saved JSON configuration files were restored.\n"
            ...
            print(
                ...
                f"{interaction.user} — {count} configs"
            )
