...
SERVER_MEMBER_ROLE_NAME = "Member"
...
def normalize_data(data):
    ...
    for guild_id in list(guilds.keys()):
        ...
        if not isinstance(
            guild_data.get("pending_invites"),
            dict
        ):
            guild_data["pending_invites"] = {}
...
class ClanInviteUserSelect(
    ...
    def __init__(
        ...
        super().__init__(
            placeholder="Select the member to invite",
            min_values=1,
            max_values=1
        )
    ...
    async def callback(
        ...
        if not clan or not is_clan_leader(
            ...
            await interaction.response.send_message(
                "❌ Only clan leaders can invite members.",
                ephemeral=True
            )
        ...
        data["pending_invites"][invite_id] = {
            "invite_id": invite_id,
        ...
        try:
            ...
            await target.send(
                embed=discord.Embed(
                    title=f"📨 Clan Invitation — {clan['clan_name']}",
                    description=(
                        f"You have been invited to join "
                        f"**{clan['clan_name']}** by "
                        f"{interaction.user.mention}.\n\n"
                        "Accept the invitation below to become a "
                        "clan member."
                    ),
                    color=discord.Color.blurple()
                ),
                view=ClanInvitationView(
                    interaction.guild.id,
                    invite_id
                )
            )
            ...
            await interaction.response.send_message(
                (
                    "❌ I could not send the invitation by DM. "
        ...
        await interaction.response.send_message(
            f"✅ Invitation sent to {target.mention} by DM.",
            ephemeral=True
        )
...
class ClanInvitationView(
    ...
    async def _get_invitation(
        ...
        invitation = data["pending_invites"].get(
            self.invite_id
        )
        ...
        return data, invitation, guild

    @ui.button(
        label="Accept Invitation",
        style=discord.ButtonStyle.success,
        emoji="✅",
        custom_id="clan_invitation_accept"
    )
    async def accept(
        self,
        interaction,
        ...
        if interaction.guild is not None:
            ...
            await interaction.response.send_message(
                "❌ Please accept the invitation from your DM.",
                ephemeral=True
            )

            return

        data, invitation, guild = await self._get_invitation(
            interaction
        )
        ...
        if guild is None:
            ...
            await interaction.response.send_message(
                "❌ The server for this invitation is unavailable.",
                ephemeral=True
            )
        ...
        if int(
            ...
            await interaction.response.send_message(
                "❌ This invitation was sent to another member.",
                ephemeral=True
            )
        ...
        server_member_role = discord.utils.get(
            guild.roles,
            name=SERVER_MEMBER_ROLE_NAME
        )
        ...
        try:
            ...
            await member.add_roles(
                clan_role,
                reason="Accepted direct clan invitation"
            )

            if server_member_role:
                await member.add_roles(
                    server_member_role,
            ...
            await interaction.response.send_message(
                f"❌ Could not assign the required roles: `{error}`",
                ephemeral=True
            )
        ...
        await interaction.response.send_message(
            (
                f"✅ You accepted the invitation and joined "
                f"**{clan['clan_name']}**!\n\n"
                "You now have the clan member role."
            )
        )
    ...
    @ui.button(
        ...
        style=discord.ButtonStyle.danger,
        emoji="❌",
        custom_id="clan_invitation_decline"
    )
    async def decline(
        self,
        interaction,
        ...
        if interaction.guild is not None:
            ...
            await interaction.response.send_message(
                "❌ Please handle the invitation from your DM.",
                ephemeral=True
            )
...
class ClanJoinView(
    ...
    async def join(
        ...
        await interaction.response.send_message(
            "❌ Clan joining is now handled through direct invitations.",
            ephemeral=True
        )
...
class Clan(
    ...
    async def on_ready(
        ...
        for guild_data in clan_data.get(
            ...
            # Restore pending direct invitation buttons
            for invite_id, invitation in guild_data.get(
                "pending_invites",
                {}
            ).items():
    ...
    @app_commands.command(
        name="announce",
        description="Send an announcement to your clan members"
    )
    @app_commands.describe(
        message="The announcement to send to your clan"
    )
    async def announce(
        self,
        interaction,
        ...
        if not clan or not is_clan_leader(
            ...
            await interaction.response.send_message(
                "❌ Only clan leaders can use /announce.",
                ephemeral=True
            )
        ...
        await channel.send(
            content=member_role.mention,
            embed=discord.Embed(
                title=f"📢 {clan['clan_name']} Announcement",
                description=message,
                color=discord.Color.orange(),
                timestamp=discord.utils.utcnow()
            ),
        ...
        await interaction.response.send_message(
            "✅ Announcement sent to the clan members.",
            ephemeral=True
        )
